...
function getEngineResponse(queryOptions, _ref12) {
  var height = _ref12.height;
  return sendQuery(queryOptions, _queryHelper.default.addQuery(_queryHelper.default.emptyQuery(), 'getEngineResponse', {
    height: height
  }), function (resolve, reject, responseName, response) {
    if (responseName !== 'ENGINE_RESPONSE') {
      var error = JSON.stringify(response.toObject().errorResponse);
      return reject(new Error("Query response error: expected=ENGINE_RESPONSE, actual=".concat(responseName, "\nReason: ").concat(error)));
    }
